...
display(fzero(@(x) f(x, 1/2), -10));
display(fzero(@(x) f(x, 1/2), 0));
...
% General Solution:                         y = c * exp(x^3 - sin(x))
% Particular Solution for y(0) = 1:         y = exp(x^3 - sin(x)), c = 1

f = @(x) exp(x.^3 - sin(x));                     % Particular solution for y(0) = 1

xvals = linspace(-1.5, 1.25, 100);              % 100 points equally spaced from -2 to 2, inclusive
yvals = zeros(size(xvals));                      % Preallocating memory for y values

for j = 1:length(xvals)
   yvals(j) = f(xvals(j));    %Compute y values for x values with a guess of 
end
...
% Solutions where x and y are positive approach positive infinity meaning
% the function increases without bound as x approaches infinity. 
% Solutions where x is negative and y is positive approach positive
% infinity meaning the function increases without bound as x approaches negative infinity.
% As x approaches 0 from either side the slope of the function goes to 0

% Solutions where y is negative approach 0 as x approaches positive or
% negative infinity

% Every solution is symmetrical about the y axis because the only y term has an
...
f_g = @(u,t) 1/2 * atan(u) + 1/4 * log(u + 1) - 1/4 * log(u-1) - t +c     % General solution
f_p = @(u,t) 1/2 * atan(u) + 1/4 * log(u + 1) - 1/4 * log(u-1) - t +c     % Particular solution at u(0) = 0.1
