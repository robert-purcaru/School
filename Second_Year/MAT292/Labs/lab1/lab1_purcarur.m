...
h = (abs(-3-7))/(N-1);              %Incrememnt between x values
x = zeros(N, 1);                    %Preallocate a column vector of Nx1 zeroes
for i = 1:N
    x(i) = ((i-1) * h) - 3;         %Generates N values starting at -3 with h difference
end

y = zeros(N, 1);                    %Preallocate the y values array
for j = 1:N
    y(j) = abs(x(j)) * x(j);        %Generate y(j) abs(x(j))*x(j) values for corresponding x(j) values
end
...
N = 5000001;                        %Number of elements we want
h = (abs(-2-3))/(N-1);              %Incrememnt between x values
x = zeros(N, 1);                    %Preallocate a column vector of Nx1 zeroes
for i = 1:N
    x(i) = ((i-1) * h) - 2;         %Generates N values starting at -3 with h difference
end

clear y;                            %Clearing y

tic;                                %Start timer
A = 5000001;                        %Number of elements we want
y = zeros(A,1);                     %Declare Nx1 vector of zeros
for j = 1:A;
    y(j) = atan(x(j));              %Assinging y to it's appropriate arctan value through iteration.
end
toc;                                %End timer

clear y;                            %Clearing y
tic;                                %Starting timer 
y = atan(x);                        %Comptuting arctan of x values through vectorization
toc;                                %End timer
...
t = linspace(1, 5, 100);                                            %Assign independent variable 100 values on [1,5]
                                                                    %From solving by hand, we find the general solution:
                                                                    %y = t*(exp(-t^2)) + c*(exp(-t^2))
y_a = (t./exp(t.^2)) + (-(1+exp(1))./exp(t.^2));                    %Assign values for y at particular solution y(1) = -1
y_b = (t./exp(t.^2)) + ((-1)./exp(t.^2));                           %Assign values for y at particular solution y(1) = 0
y_c = (t./exp(t.^2)) + ((1+exp(1))./exp(t.^2));                     %Assign values for y at particular solution y(1) = 1

plot(t,y_a,t,y_b,t,y_c)                                             %Plot all 3 particular solutions against time
xlabel('t');
ylabel('y(t)');
title('Particular solutions to y'' + 2ty = exp(-t^2).');
legend('y(1) = -1', 'y(1) = 0', 'y(1) = 1', 'Location','North East');
...
f = @(x) (x.^3 - abs(x)) .* log(x.^2+1);    %Declaration of inline function |f(x) = (x^3 - abs(x)) * ln(x^2+1)|
disp(f(0));                                 %Computing and displaying f(0)
disp(f(1));                                 %Computing and displaying f(1)
disp(f(-1));                                %Computing and displaying f(-1)

x = linspace(-5,5,100);                     %Generating x 100 equally spaced values on [-5,5]

plot(x, f(x))                     %Plot f(x) for x = [-5,5]
xlabel('x');
ylabel('f(x)');
title('Graph of f(x) = (x^3 - abs(x)) * ln(x^2+1) on x = [-5,5]');
legend('f(x) = (x^3 - abs(x)) * ln(x^2+1)', 'Location','South East');
...
f = @(x,y) y + y.^3 - cos(x) + x.^2 - x.^4;     %Declaration of function f(x,y) = y + y^3 - cos x + x^2 - x^4

disp(f(0,0));                                   %Computing and displaying f(0,0)           
disp(f(pi/2, 1));                               %Computing and displaying f(pi/2,1)
disp(f(-1,-1));                                 %Computing and displaying f(-1,-1)
...
display(fzero(@(y) f(0, y), 5));
display(fzero(@(x) f(x, 1/2), 1.2));
display(fzero(@(x) f(x, 1/2), 0.3));
...
xvals = linspace(-2,2,100);                     % 100 points equally spaced from -2 to 2, inclusive
yvals = zeros(size(xvals));                     % Preallocating memory for y values

for j = 1:length(xvals)
   yvals(j) = fzero(@(y) f(xvals(j), y), 0);    % Compute y values for x values with a guess of 0
end

plot(xvals, yvals);
title('Solution to f(x,y) = y + y^3 - cos x + x^2 - x^4 over x = [-2,2]');
legend('f(x,y) = y + y^3 - cos x + x^2 - x^4 ', 'Location', 'North');
xlabel('x');
ylabel('y');
%% Exercise 10
...
% General Solution:                         ln(y) = x^3 - sin(x) + c
% Particular Solution for y(0) = 1:         ln(y) = x^3 - sin(x)                  
f = @(x, y) x.^3 - sin(x) - log(y);                  % Particular solution for y(0) = 1 

xvals = linspace(-1.5, 1.25, 100);                  % 100 points equally spaced from -2 to 2, inclusive
yvals = zeros(size(xvals));                         % Preallocating memory for y values

for j = 1:length(xvals)
   yvals(j) = fzero(@(y) f(xvals(j), y), 1.1);         %Compute y values for x values with a guess of 
end

plot(xvals, yvals);
title('Graph of particular solution to dy/dx = (-cos x + 3x^2) y for y(0) = 1.');
xlabel('x');
ylabel('y');
legend('y = exp(x^3 - sin(x))', 'Location', 'South East');
%% Install iode
...
% The general solution is y = -2*((x^2+c)^-1). If the constant is > 0, as x approaches infinity
% the funciton is continuous along all x and approaches y = 0 as x
% approaches infinity. If c < 0, the starting position of the solution is
% more important, if the starting position is in quadrant 1 or 2, y
% approaches infinity as x approaches sqrt(c). If it is in the 3rd
% quadrant, y approaches negative infinity as x approaches +/-sqrt(c). If the
% starting position is in the 4th quadrant, y approaches infinity as x
% approaches infinity.
% If c = 0, the solution is the same for quadrants 3 and 4, however there
% is nowhere in the domain that gives y as a positive number.

% Every solution is symmetrical about the y axis because the only x term of every solution has an
% even exponent (2), meaning that y is an even function with the property
% y(x) = y (-x) for every x.
%% Exercise 12
...
% As x appraoches positive infinity: y_0 values in the range (-1,2)
% approach y = 1, y_0 = 2 approaches y = 2, y_0 <= -1 approaches y = -1,
% and y > 2 approaches positive infinity
% 
% As As x appraoches negative infinity: y_0 > 1 approach y = 2, y_0 = 1
% appraoches y = 1, y y_0 in the range [-1,1) appraoch y = -1 and y < -1
% approach negative inifinity
...
%fzero doesn't work because the sign of the function is determined by which
%version of the function is being used - the version with a positive or
%negative sign. Since fzero can't find a solution because the function ends
%at 0 in both cases so it can't overshoot and close in on 0.

f = @(x,y) x.^2 + y.^2 - 4              % particular solution in implicit form 
y_pos = @(x) sqrt(4-x.^2)               % particular solution in explicit form for positive y values
y_neg = @(x) -sqrt(4-x.^2)              % particular solution in explicit form for negative y values
xvals = linspace (-2,2,50);             % 50 points equally spaced from -2 to 2, inclusive. this range is used because the figure is a circle of radius 2
pos_yvals = zeros(size(xvals));         % Preallocating memory for positive y values
neg_yvals = zeros(size(xvals));         % Preallocating memory for negative y values

for j = 1:size(xvals)
    pos_yvals(j) = y_pos(xvals(j));      % Assigning positive y values 
end

for j = 1:size(xvals)
    neg_yvals(j) = y_neg(xvals(j));      % Assigning negative y values
end

pos_yvals = y_pos(xvals);
neg_yvals = y_neg(xvals);

plot(xvals, pos_yvals, xvals, neg_yvals);
...
% Particular solution at u(0) = 0.1
f_p = @(t,u) -0.5*atan(u) - 0.25*log(u + 1) + 0.25*log(1-u) + t + 0.1;   

t = linspace(0,1, 50);
u = zeros(size(t));

for j = 1:length(t)
    u(j) = fzero(@(u)f_p(t(j), u), 0.5);
end
